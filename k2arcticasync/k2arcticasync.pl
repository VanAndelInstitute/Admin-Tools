...
#  Description: 
# do a parallel sync the arctica k2 cryoem data from the instrument storage to the permanant storage
...
my $WORKDIR = "/varidata/research/admin/tools/k2arcticasync/logs/$TIME";
system("mkdir $WORKDIR");
my $listFile = "$WORKDIR/$TIME";
my $listFileParts = $TIME . "PART";
my $SRC = "/remote/k2arctica/Dosefractions/";
my $DEST = "/varidata/research/instruments/cryoem/k2arctica/Dosefractions";
#################


#make sure this is not already running, die if we are. NOTE: pgrep truncates finding of the process name!
my $numprocs = `pgrep k2arcticasync.p | wc -l`;
chomp $numprocs;
